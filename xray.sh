...
if [ $TIPO -eq 1 ]; then
  sed -i 's/"summary": "TITULOJSON"/"summary": "Resultado Prueba Automatizada ON-DEMAND "/g' integracion/jira.json
else
  sed -i 's/"summary": "TITULOJSON"/"summary": "Resultado Prueba Automatizada PIPELINE "/g' integracion/jira.json
fi
